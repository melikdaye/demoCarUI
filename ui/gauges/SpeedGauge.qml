...
Rectangle {
        ...
        property real currentSpeed: speedController.currentSpeed
        color: "transparent"
        ...
        Canvas {
           ...
           onPaint: {
               ...
               var angle = ((currentSpeed < 1 ? currentSpeed+1 : currentSpeed) / maxSpeed) * Math.PI;
               ctx.beginPath();
               ctx.moveTo(width / 2, height / 2);
               ctx.arc(width / 2, height / 2, width / 2 - 10, 0.75 * Math.PI , 0.75 * Math.PI + angle);
               ctx.strokeStyle = "rgba(0, 256, 0, 1)";
               ctx.lineWidth = 2;
                 ctx.stroke();
               let gradient = ctx.createConicalGradient(width/2 , height/2, 0.75 * Math.PI + Math.PI);
               gradient.addColorStop(1, "rgba(0, 128, 0, 0)"); // Transparent black
               gradient.addColorStop(0, "rgba(0, 256, 0, 0.75)"); // Transparent green
...
//              ctx.fillStyle = "rgba(0, 128, 0, 0.5)";
               ...
               // Draw the polar line with gradient color
                           var startX = width / 2;
                           var startY = height / 2;
                           var endX = width / 2 + Math.cos(0.75 * Math.PI + angle) * (width / 2 - 10);
                           var endY = height / 2 + Math.sin(0.75 * Math.PI + angle) * (width / 2 - 10);
               ...
               // Draw the main ticks
                               ...
                               for (var i = 0; i <= numMainTicks; i++) {
                                   var tickAngle = 0.75 * Math.PI + (i / numMainTicks) *  Math.PI;
                               ...
                               for (let i = 0; i < numMainTicks; i++) {
                                   var startMinorAngle = 0.75 * Math.PI;
               ...
               // Draw the current speed in the center
                             ...
                             ctx.fillText(currentSpeed.toFixed(0), width / 2, height / 2);

                               ctx.font='20px "%1"'.arg(kanit.name)
                               ctx.fillStyle = "rgba(0, 256, 0, 0.75)";
                               ctx.textAlign = "center";
                               ctx.textBaseline = "middle";
                               ctx.fillText("KM/H", width / 2, height / 2 + 75);
        ...
        Connections {
               ...
               onCurrentSpeedChanged: speedCanvas.requestPaint()
           }
